...
export default function QuoteEditForm({
  ...
  const [totalAmount, setTotalAmount] = useState<number>(quote.amount);
  ...
  const onSubmit = async (data: QuoteFormData) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/sales-opportunities/${salesOpportunityId}/quotes/${quote.id}`,
        {
          method: "PATCH",
          headers: {
      ...
      if (!response.ok) {
        ...
        console.error("見積更新エラー:", errorData);
        // 詳細なエラーメッセージを表示
        const errorMessage = errorData.details
          ? `${errorData.error || "見積の更新に失敗しました"}\n${JSON.stringify(errorData.details, null, 2)}`
          : errorData.error || "見積の更新に失敗しました";
      ...
      setError(
        err instanceof Error ? err.message : "見積の更新に失敗しました"
      );
    } finally {
