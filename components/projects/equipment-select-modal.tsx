...
interface EquipmentCreateFormWithDefaultsProps {
  companyId: string;
  projectId: string;
...
function EquipmentCreateFormWithDefaults({
  companyId,
  projectId,
  onSuccess,
  onCancel,
}: EquipmentCreateFormWithDefaultsProps) {
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
        ...
        <div className="sm:col-span-2">
          <LookupField
            ...
            filterParams={watch("companyId") ? { companyId: watch("companyId") } : {}}
            error={errors.projectId?.message}
            className={!watch("companyId") ? "opacity-60" : ""}
          />
        ...
        {/* 製造番号 */}
        <div>
          <label htmlFor="serialNumber" className="block text-sm font-medium text-gray-900">
            製造番号
          </label>
          <input
            type="text"
...
export default function EquipmentSelectModal({
  ...
  const searchEquipment = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // 既にこのプロジェクトに紐付いている機器を除外
        const availableEquipment = (data.equipment || []).filter(
          (eq: Equipment) => !eq.project || eq.project.id !== projectId
        );
  ...
  }, [debouncedSearchQuery, page, limit, companyId, projectId]);
  ...
  }, [isOpen, debouncedSearchQuery, page, searchEquipment]);
        ...
        >
          ...
          <div className="border-b border-gray-200 px-6 py-4">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="機器名、機種、製造番号、設置場所で検索..."
                value={searchQuery}
                onChange={(e) => {
                        ...
                        >
                          <td className="px-4 py-4 whitespace-nowrap">
                            <input
                              ...
                              onClick={handleClick}
                              onChange={(e) =>
                                handleSelectOne(eq.id, e.target.checked, index)
                              }
                              className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                            />
                          </td>
        ...
        <div className="fixed inset-0 z-[60] overflow-y-auto">
          <div className="flex min-h-full items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col">
              {/* ヘッダー */}
              <div className="flex items-center justify-between border-b border-gray-200 px-6 py-4">
                <h2 className="text-xl font-semibold text-gray-900">機器を新規作成</h2>
                <button
                  onClick={() => setIsCreateModalOpen(false)}
                  className="text-gray-400 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 rounded"
                >
              ...
              <div className="flex-1 overflow-y-auto px-6 py-4">
                <EquipmentCreateFormWithDefaults
                  companyId={companyId}
                  projectId={projectId}
