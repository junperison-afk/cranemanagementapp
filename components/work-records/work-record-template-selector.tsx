...
interface WorkRecordTemplateSelectorProps {
  isOpen: boolean;
  onClose: () => void;
  workRecordId: string;
  onSuccess?: () => void;
...
export default function WorkRecordTemplateSelector({
  isOpen,
  onClose,
  workRecordId,
  onSuccess,
}: WorkRecordTemplateSelectorProps) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await fetch(
        `/api/work-records/${workRecordId}/generate-document`,
        {
          method: "POST",
          headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || "作業記録の生成に失敗しました");
      }
      ...
      console.error("作業記録生成エラー:", err);
      setError(err instanceof Error ? err.message : "作業記録の生成に失敗しました");
    } finally {
      ...
      <div className="flex min-h-screen items-center justify-center p-4">
        ...
        <div className="relative bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col">
          ...
          <div className="flex items-center justify-between p-6 border-b border-gray-200">
            <h2 className="text-xl font-semibold text-gray-900">作業記録印刷</h2>
            <button
              onClick={onClose}
            ...
            {isGenerating && (
              <div className="absolute inset-0 bg-white bg-opacity-75 flex items-center justify-center z-10">
                <div className="text-center">
                  ...
                  <p className="text-lg font-medium text-gray-900">作業記録を印刷中...</p>
